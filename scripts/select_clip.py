...
def classify_with_vision(
    ...
    prompt = (
        ...
        "請判斷並只回傳 JSON（不含說明）：\n"
        "{\n"
        '  "category": "車禍" | "警匪槍戰" | "社會事件" | "其他"（看不出明確類別就填「其他」，不要硬套）,\n'
        '  "event_summary": "一句話客觀描述畫面『看得到』的主體、場景與動作（繁體中文）。'
        '⚠️只寫畫面確實有的東西，絕對不要推論看不到的劇情、因果或前因後果——'
        '不確定是不是車禍/案件，就直接描述看到什麼（例：夜間道路上一輛受損車輛被拖行、'
        '路邊停著一輛小貨車），不要寫「疑似發生車禍」「散落碎屑」「案發後」這種腦補",\n'
...
def _json_default(o):
    """numpy 純量（bool_/int64/float64…）json 不認 → 轉回原生 Python 型別。
    避免某條分析路徑不小心把 numpy 值塞進結果，害整份快取寫入炸掉。"""
    if hasattr(o, "item"):
        return o.item()
...
def _cache_put(namespace: str, video: Path, value: dict):
    key = f"{namespace}:{_cache_key(video)}"
    try:
        cache = _cache_load()
        cache[key] = value
        # default=_json_default：把 numpy 型別轉原生，序列化不再因 numpy bool 炸掉。
        # 先序列化成字串再寫檔——序列化成功才算數。
        payload = json.dumps(cache, ensure_ascii=False, default=_json_default)
        _CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        _CACHE_FILE.write_text(payload, encoding="utf-8")
    except OSError:
        pass  # 寫檔失敗不影響主流程（記憶體快取仍有效）
    except Exception:
        # 極端非預期型別導致序列化失敗 → 移除剛加的壞值，避免污染 in-memory 快取
        # 連累同一次產製後面每一支素材的快取寫入（原本的惡性 bug）
        try:
            _cache_load().pop(key, None)
        except Exception:
            pass
...
def match_narration_to_clips(
    ...
    prompt = (
        ...
        "4-1. ⚠️⚠️ 優先用不同素材、避免過度重複同一支：畫面要有變化，同一支影片反覆出現觀眾會膩。"
        "**只要還有「同場景、跟事件相關」的素材沒被用到，就先穿插進來補變化**——例如港口/碼頭的"
        "搬運貨物、人員作業、周邊環境等現場畫面，就算旁白沒指名講到，也可以拿來當同場景旁白的襯底，"
        "**優先於重複同一顆鏡頭**。不要讓一支影片用了三四次、別支切題素材卻完全沒用到。"
        "只有真的沒有其他相關素材時才重複；開場連續兩三段尤其不要都用同一支影片\n"
        "5. ⚠️⚠️ 受訪/講話特寫（標記「有原音人聲」或描述含「受訪」「對鏡頭說話」）"
        "**絕對不要拿來當旁白的無聲底圖**——不管有沒有其他畫面可用都一樣。這種畫面只能"
        "出現在它自己的受訪原音段（SOT，讓當事人用原音講）；不是 SOT 就完全不要選它。"
        "敘事性旁白（講事件現場經過，如爆管噴水、淹水、搶修、案發、救援）一律配「現場實況／"
        "事件主體」畫面或現場照片，呼應現場此刻正在發生什麼。若真的沒有現場/主體畫面可用，"
        "**寧可用現場照片、重複主體畫面、甚至補黑幕，也不要用受訪畫面墊旁白**\n"
        "5-3. ⚠️ 同一位受訪者的講話畫面在整支成片裡不要重複出現在多個旁白段——"
        "他應該只在自己的受訪原音段（SOT）露臉一次；其餘時間用現場實況/主體畫面/照片\n"
        ...
        "5-0b. ⚠️⚠️ **開場第一段要挑「最有張力、一眼看懂事件」的關鍵畫面**——研究指出短影音"
        "50~60% 觀眾在前 3 秒流失，開場那幾秒是全片最貴的畫面。在符合破題旁白、且符合 5-0 的"
        "前提下，從所有可用素材裡優先挑視覺衝擊最強、最能代表這則新聞的那一段（例：撞擊/翻覆"
        "後的車損、淹水沖刷的缺口、大火與濃煙、坍方落石、警方攻堅、群眾衝突），不要開在平淡的"
        "空景、遠景鋪陳或事件發生「之前」的畫面。取用秒數要正好落在那個關鍵畫面上——若某段"
        "描述顯示精彩處在中後段，就從精彩處前 1 秒開始取，別把重點切掉只給到事發前的鋪陳\n"
        ...
        "6. ⚠️ 同一支影片被用第二次以上時（不論相不相鄰），每一次都要取「明顯不同的時段」，"
        "不要抓到相近或重疊的秒數（否則等於同一個畫面重播）。做法：第一次用前段、第二次就用"
        "中段或後段，讓每次露出的畫面內容都不一樣；同一支影片的兩段起點至少錯開一個片段的長度。"
        "例：上一段用了 V1 的 1~6 秒，下次要用 V1 就從 12 秒之後取，不要再取 3~8 秒\n"
        ...
        "7-0. ⚠️ 記者拍的新聞照片是「一等素材」，不是只有沒影片才用的墊檔：照片若清楚拍到"
        "事件主體或現場（新聞配圖常是最具代表性的一張），就可以**主動穿插**進來當某段旁白的"
        "畫面，跟影片交替使用增加變化與資訊量。**尤其影片素材少、或同一支影片快要重複用到時，"
        "優先改用一張切題的照片，勝過重複同一段影片**。但一定要遵守 7-1：**每張照片整支只用一次**——"
        "有幾張不同的好照片就穿插幾張（各一次）；只有一張照片就只穿插一次，絕不把同一張用兩次\n"
